...
- `ShowLogsCommand` exige modo administrador e e-mail permitido de suporte;
...
O subsistema de logs combina armazenamento central, leitura de dashboard e restrições de suporte.
...
- os logs são visíveis apenas para e-mails de suporte autorizados em modo administrador;
- os arquivos de log são organizados por máquina e data;
- o dashboard mostra métricas, eventos e erros técnicos detalhados;
- entradas históricas de log são dados já gravados, portanto a mudança de idioma da UI não reescreve textos antigos.
